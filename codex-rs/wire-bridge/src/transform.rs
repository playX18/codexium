...
use crate::common::append_reasoning_content;
use crate::common::extract_reasoning_field_text;
use crate::common::extract_reasoning_summary_text;
use crate::common::response_function_call_item;
use crate::common::response_function_call_item_with_namespace;
use crate::common::split_leading_think_block;
use crate::error::BridgeError;
use crate::json_canonical::canonical_json_string;
use crate::json_canonical::canonicalize_json_string_if_parseable;
use crate::json_canonical::canonicalize_tool_arguments;
use crate::json_canonical::short_sha256_hex;
...
#[allow(dead_code)]
pub fn responses_to_chat_completions(body: Value) -> Result<Value, BridgeError> {
...
pub fn responses_to_chat_completions_with_reasoning(
    ...
    if let Some(max_tokens) = body.get("max_output_tokens") {
        if crate::model_helpers::is_openai_o_series(model) {
    ...
    crate::model_helpers::inject_openai_stream_include_usage(&mut result);
...
fn apply_reasoning_options(
    ...
    let Some(config) = config else {
        if crate::model_helpers::supports_reasoning_effort(model)
...
#[allow(dead_code)]
pub fn chat_completion_to_response(body: Value) -> Result<Value, BridgeError> {
...
pub fn chat_completion_to_response_with_context(
    ...
    let choices = body
        .get("choices")
        .and_then(|v| v.as_array())
        .ok_or_else(|| BridgeError::TransformError("No choices in chat response".to_string()))?;
    let choice = choices
        .first()
        .ok_or_else(|| BridgeError::TransformError("Empty choices in chat response".to_string()))?;
    let message = choice
        .get("message")
        .ok_or_else(|| BridgeError::TransformError("No message in chat choice".to_string()))?;
...
fn response_tool_search_call_item(
    ...
    crate::common::attach_optional_reasoning_content_field(&mut item, reasoning);
    item
}
...
mod tests {
    ...
    fn responses_request_to_chat_maps_openrouter_to_native_reasoning_object() {
        ...
        // thinking_param=none: OpenRouter does not accept thinking:{type}.
    ...
    #[test]
    fn responses_request_to_chat_omits_thinking_disabled_for_generated_provider_config() {
        let config = CodexChatReasoningConfig {
