...
**1. Assign a task name before dispatching.** Before the implementer runs, pick a canonical `task_name` for the plan task (lowercase letters, digits, underscores — e.g. `hook_install`). Dispatch the implementer with `spawn_agent`, passing that `task_name` and a prompt built from `./implementer-prompt.md`. Refer to the spawned agent by its canonical task name. Do NOT treat the task complete here — completion is gated on review (step 4).
...
passes, call `wait_agent` on the implementer's task name to confirm completion before moving to the next plan task.
...
Model choice is automated — set `model` and `reasoning_effort` on every `spawn_agent` call by default. Users may optionally override via `/compose-models` or by stating a preference. Do not ask unless they opt in.

Use the least capable model that can handle each role:

| Role / signal | `model` | `reasoning_effort` |
|---|---|---|
| Mechanical implementer (1–2 files, complete spec) | `gpt-5.4-mini` or `gpt-5.5` | `low` or `medium` |
| Integration / judgment implementer | `gpt-5.4-mini` or `gpt-5.5` | `medium` or `high` |
| Architecture, design, ambiguous spec | `gpt-5.5` | `medium`, `high`, or `xhigh` |
| Quick exploration, small fixes, logs, UI tweaks, diff/test loops | `gpt-5.3-codex-spark` | default |
| Spec or code reviewer | ≥ implementer's model | ≥ implementer's effort |

**Spark (`gpt-5.3-codex-spark`):** fast mode — latency over depth. Use for small edits, code scanning, log/debug clues, UI tweaks, simple API propagation, parallel explore-while-you-plan workers, and quick diff/review/test loops. Never Spark for architecture, complex refactors, subtle bugs, spec gates, or reviewers.

Only use slugs present in the catalog. If `gpt-5.3-codex-spark` is unavailable, use `gpt-5.4-mini` or `gpt-5.5` with low effort for spark-tier work.
...
[Assign task_name `hook_install` for Task 1]

Task 1: Hook installation script

[Get Task 1 text and context (already extracted)]
[spawn_agent with task_name "hook_install" and implementer prompt from ./implementer-prompt.md]
...
[Gate passed → wait_agent on hook_install, proceed to Task 2]
...
[Gate passed → wait_agent on recovery_modes]
